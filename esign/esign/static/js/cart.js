...
function addtocart (prod_id, prod_num, prod_name, prod_price)
{
	...
	if (id_check == 0)
	{
	    ...
	    /*var new_form = $("<form>", {"class" : "item_form"}).appendTo(new_item);
	  	...
	  	var new_form_html = '<form class="item_form" data-parsley-validate><input type="hidden" value="'
	  	+getCookie('csrftoken')+'" name="csrfmiddlewaretoken"><div class="order_txt">'+prod_name+' - '
	  	+prod_price+'р.</div>количество: <input class="order_cnt" name="'+prod_id+'" value='+prod_num
	  	+' type="text" data-parsley-type="digits" data-parsley-trigger="keyup"><input class="change_cnt" type="button" value="поменять"><input class="remove_order" type="button" value="убрать"></form>';	    new_item = new_item.append(new_form_html);
